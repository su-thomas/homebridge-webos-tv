...
class LgTvController extends EventEmitter {
  ...
  setPictureMode(pictureMode) {
   ...
   // expert1-hdmi2 becomes expert1, only on hdmi2
    ...
    let onlyOnInput = pictureMode.split('-')[1];
    if (onlyOnInput) {
      onlyOnInput = "com.webos.app." + onlyOnInput
    }
    ...
    switch(pictureMode) {
      ...
      case "gameDarkRoomLevel2":
        ...
        break;
      default:
        if (!onlyOnInput || onlyOnInput === currentInput) {
          this.setPictureSettings({pictureMode: pictureMode});
          this.logInfo("onlyOnInput: ", onlyOnInput, "currentInput: ", currentInput)
        } else {
