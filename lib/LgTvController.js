...
class LgTvController extends EventEmitter {
  ...
  setOtherSettings(settings) {
    this.setSystemSettings("other", settings);
  }
  ...
  setPictureMode(pictureMode) {
    ...
    */
   // expert1-hdmi2 becomes expert1, only on hdmi2
    let currentInput = this.getForegroundAppAppId();
    let special = pictureMode.split('-')[1];
    let onlyOnInputs = [];
    if (special) {  
      let inputs = special.split(',');
      for (let i = 0; i < inputs.length; i++) {
        onlyOnInputs.push("com.webos.app." + inputs[i]);
      }
    }

    pictureMode = pictureMode.split('-')[0];
    
    switch(pictureMode) {
      case "gameDarkRoomOff":
        this.setOtherSettings({darkMode: "off"});
        this.logInfo("Setting game optimiser darkMode off")
        break;
      case "gameDarkRoomLevel1":
        this.setOtherSettings({darkMode: "level1"});
        this.logInfo("Setting game optimiser darkMode level1")
        break;
      case "gameDarkRoomLevel2":
        this.setOtherSettings({darkMode: "level2"});
        this.logInfo("Setting game optimiser darkMode level2")
        break;
      default:
        if (!onlyOnInputs || onlyOnInputs.includes(currentInput)) {
          this.setPictureSettings({pictureMode: pictureMode});
          this.logInfo("onlyOnInputs: ", onlyOnInputs, "currentInput: ", currentInput)
        } else {
          this.logInfo("Not setting picture mode because current input is skipped")
        }
    }
  }
